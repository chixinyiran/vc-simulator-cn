...
const Sfx = (function(){
  ...
  const lib={
    ...
    // 封存确认：银钱洒落(浑厚版)——深低频铜钟打底 + 中低频金属额颁洒落 + 克制高频亮片,厚重有分量
    confirm(){
      // ① 深低频铜钟/盖章打底(浑厚感的根:更低、更响、更长的低音共鸣)
      tone(73, 0, 0.42, 'sine', 0.85, 55);     // 极低频沉底(厚)
      tone(110, 0, 0.38, 'sine', 0.7, 82);     // 低频主体
      tone(146, 0.01, 0.34, 'triangle', 0.5, 110); // 中低频铜色(增厚度)
      // ② 金属额颁洒落:降到中低频段(1100-2100Hz,去掉刺耳高频,保留金属质感),额颁加重加长
      var coins=[1318,1568,1175,1397,1760,1047,1480,2093];
      for(var i=0;i<coins.length;i++){
        var t=0.05+i*0.04+Math.random()*0.022;
        var pk=0.34+Math.random()*0.16;
        tone(coins[i], t, 0.22, 'triangle', pk);        // 主额颁(中低频,厚)
        tone(coins[i]*0.5, t+0.004, 0.14, 'sine', pk*0.5); // 低八度共鸣增厚(关键:下添不是上添)
      }
      // ③ 尾部余韵(克制高频,只一点点点缀,不抢浑厚主体)
      tone(2349,0.40,0.20,'sine',0.12);
    },
